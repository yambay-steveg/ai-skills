...
> ⚠️ **DRAFT — not yet tested/verified (2026-06-27).** This AGENTS.md was derived from
> `CLAUDE.md` and adapted for Codex. The Codex-specific bits — `~/.codex/…` paths and
> `codex …` CLI commands — have **not been verified** against the actual Codex CLI. Treat it as
> a starting point and confirm commands before relying on them.

Personal repository for building, testing, and sharing Codex skills.

## Purpose

- Build custom Codex skills (SKILL.md + optional Python scripts)
...
Previous manual installs (`~/.codex/skills/email/`, `files/`, `md-to-word/`) were removed on 2026-04-01 to avoid duplicates.

## Shared M365 Configuration

All M365-connected skills share auth config at `~/.codex/m365/`:
...
**Graph API config** has been added to the global `~/.codex/AGENTS.md` so Codex always uses the correct app for ad hoc M365 work.
...
```bash
codex plugin marketplace add yambay-tech/ai-assistants   # Add marketplace (one-time)
codex plugin install <plugin>@yambay-tech --scope user    # Install a plugin
```

Auto-update is enabled in `~/.codex/plugins/known_marketplaces.json`.
...
- `skills/` — Custom Codex skills authored in this repo
  - `_template/` — Copy this to start a new skill (SKILL.md + scripts/ + tests/)
  - `email-tidy/` — SaneBox folder triage skill
  - `session-search/` — Search and resume past Codex sessions
- `raycast/` — Raycast script commands that launch Codex skills
- `warp/` — Warp terminal workflows and rules
  - `workflows/` — YAML workflow definitions
  - `rules/` — Markdown context rules
- `bin/` — Launcher scripts (`aiw`, `aip`) for Codex sessions
- `session-cards/` — the session-card system: `cardctl` launcher, spec (R1–R15), and PoC
- `profiles/` — Notes on third-party skills under evaluation
- `install.sh` — Copies a skill from this repo to `~/.codex/skills/`
- `AGENTS.md` — This file (project context for AI assistants)

## Building Skills

A Codex skill is a folder containing at minimum a `SKILL.md` with YAML frontmatter:
- `name` — skill identifier
- `description` — when to trigger (include example phrases)
- `allowed-tools` — which Codex tools the skill can use

The SKILL.md body contains instructions Codex follows. If the skill needs code, put scripts in `scripts/` and have the SKILL.md instruct Codex to call them.
...
4. `./install.sh my-skill` (copies to `~/.codex/skills/`)
...
- Token cache at `~/.codex/m365/.token_cache_skills.json` — delete to force re-auth if issues arise
...
- Skill files reference paths like `~/.codex/skills/<skill-name>/` — keep this convention
- Keep this AGENTS.md updated as skills are added, tested, or modified
